...
# zsh only sources ~/.zshrc for INTERACTIVE shells; login alone is not
# enough (see /etc/zsh/zshrc's own comment: "sourced only for interactive
# shells"). Agents and Moshi probe tools with 'zsh -lc', a login shell that
# is never interactive, so without this ~/.zshrc's PATH and 'mise activate'
# line would silently never run for them. ~/.zprofile IS read for every
# login shell regardless of interactivity, so source ~/.zshrc from there,
# guarded so a real interactive login shell (an SSH or Moshi session) does
# not source it twice.
cat >"${DEV_HOME}/.zprofile" <<'EOF'
# devbox: makes ~/.zshrc's PATH and tool activation reach non-interactive
# login shells ('zsh -lc'), which zsh does not source ~/.zshrc for on its
# own. Skipped when interactive so a real login session does not source
# ~/.zshrc twice (once here, once via zsh's normal interactive startup).
if [[ ! -o interactive ]] && [ -f "$HOME/.zshrc" ]; then
  source "$HOME/.zshrc"
fi
EOF
chown 1000:1000 "${DEV_HOME}/.zprofile"
